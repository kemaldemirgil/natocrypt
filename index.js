...
const parse_data = (chars, multiplier) => {
    ...
    chars.forEach((char) => {
        for (let i = 0; i < multiplier; i++) {
            let random_nato = nato_alphabet_list[Math.floor(Math.random() * nato_alphabet_list.length)];
            nato_text.push(random_nato);
        }
        let char_list = nato(char);
        char_list.forEach((element) => {
 ...
 * @returns
 */
const morse_encrypt = (field, multiplier) => {
    let morse_text = [];
    field.forEach((char) => {
        for (let i = 0; i < multiplier; i++) {
...
var random_property = function (obj) {
...
class Natocrypt {
     ...
     * @returns
     */
    morse(type, field, multiplier = random_multiplier(1, 10)) {
